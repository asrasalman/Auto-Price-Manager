...
public partial class pages_ShopConnect : Base
{
    ...
    private void SetAccounts()
    {
        ...
        Package package = context.Users.FirstOrDefault(f => f.User_Code == UserKey).Package;
    ...
    //                            var result = (from ua in context.UserAccounts where ua.Ebay_User_Name == ebayUserName select new { ua.User_Code }).ToList();
    //                            if (result.Count() == 0)
    //                            {
    //                                if (hfSelectedEbayAccountID.Value == "0")
    //                                {
    ...
    //                                    EbayServiceBL services = new EbayServiceBL((int)userAccount.User_Code);
    //                                    if (services.UserTokens != null)
    //                                    {
    //                                        Dictionary<string, int> localToken = new Dictionary<string, int>();
    //                                        foreach (KeyValuePair<int, string> pairs in service.UserTokens)
    //                                        {
    //                                            string ebayUserNames = service.GetUser(pair.Value);
    //                                            if (pair.Key == userAccount.User_Account_Code)
    //                                            {
    //                                                userAccount.Ebay_User_Name = ebayUserNames;
    //                                                context.SaveChanges();
    //                                            }
    //                                        }
    ...
    //                            else
    //                            {
    //                                lblAuthError.Text = "You can't connect ,Ebay user already exist !";
